 ...
 * 成"这条轨道没用过":slack 取到最大值 `W - g`,自上而下扫到它必定接纳。所以**编排起点之前
 * 超过 D 毫秒的历史,对结果没有任何贡献**,可以整段丢掉。D 取 `max(scrollDuration,
 * fixedDuration)`,两种模式统一用一个数,不各算各的。
 *
 * 这条性质只覆盖"更早的历史",不覆盖窗口起点之后的头 D 毫秒:那一段里整池编排看到的是真实
 * 的轨道占用,窗口编排看到的是空轨道,两边可能选到不同轨道。分歧的来源是起点那一瞬间,不是
 * 预热长度,所以拉长预热并不能消掉它。
 *
 * 能保证的是这三条:
 ...
 * 3. **不多丢也不少丢弹幕**。同一批弹幕仍然全部上屏,变的至多是纵向排法。
 *
 * 实际的分歧量取决于选轨规则。[CollisionFreeScheduler] 自上而下 first-fit 之后,起点处的差异
 * 不会一直传下去:轨道 0 只要让开就重新被选中,两边随即对齐——13 轨的高密度合成池上,seek
 * 之后一分钟内只有 1.1% 的弹幕落在与整池编排不同的轨道上。同一份池子在上一版的 best-fit
 * 规则下是 81%,因为那条规则是跨全部轨道的 argmin,没有可以回归的锚点。
 *
